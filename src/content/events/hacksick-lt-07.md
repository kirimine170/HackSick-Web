---
title: 第7回 HackSick LT会
description: AI・LLM、機械学習、歩容認証などをテーマに、研究と実装を往復したLT会。
startDate: 2026-07-25T18:00:00+09:00
endDate: 2026-07-25T19:00:00+09:00
location: 秋葉原No21／オンライン
format: hybrid
status: finished
tags: [Technology, AI, Research, Software]
project: hacksick-lt
registrationUrl: https://hacksick.connpass.com/event/400238/
organizers: [霧峰 雫]
featured: true
theme: AI・LLM 〜人類の労働奪ってくれました…？〜
---

牛の個体識別、LLMアーキテクチャ、歩容認証スマートロックなど、AIと研究に関する発表を共有しました。
